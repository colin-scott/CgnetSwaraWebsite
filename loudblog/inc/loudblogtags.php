...
function permalink_to_post ($content) {
    global $settings, $currentid;
    return $settings['url']."/stanford/index.php?id={$currentid}";
}
...
if (isset($att['page_index'])) { $page_index = $att['page_index']; }
		else { $page_index = 1; }
...
if (((isset($_GET['page'])) AND ($paging == "true")) OR (isset($att['page_index']))) {
    $pageTemp = isset($att['page_index']) ? $att['page_index'] : $_GET['page'];
    $start = $loops*($pageTemp-1);
} else { $start = 0; }
...
function randomimage($dir)	{//map to a permanent img
//selects a random picture to be shown in the articles
//added by Prasun Paul
...
function perm_img(){
	...
	if (!empty($_GET['id'])){
		...
		$files = glob('/home/stanford/web/loudblog/custom/templates/default/i/ppimages' . '/*.*');
		$c  = count($files);
		$img_string = 'http://cgnetswara.org/stanford/loudblog/custom/templates/default/i/ppimages/';
		$imageval = ($imageval % $c);
		$img = $files[$imageval];
		$pos = strrpos($img, '/') + 1;
...
function post_from_dlvrit(){
	if (isset($_GET['utm_source']) || isset($_GET['utm_medium'])){
		...
		$id = $_GET['id'];
		return "window.location = 'http://cgnetswara.org/stanford/index.php?id=$id';" ;
	}
	return "";
}
...
function no_of_posts_in_category($content){
	...
	//$query_to_count = "SELECT COUNT( * ) FROM ".$GLOBALS['prefix']."lb_postings WHERE tags LIKE '%MP%'";
...
function prevpost() {
	...
	$query = "SELECT id,title,image_file FROM ".$GLOBALS['prefix']."lb_postings WHERE status = 3 AND posted > '".$postings[$currentid]['posted']."'";
	if (isset($att['impact'])) {
      		$query .= "title LIKE 'Impact%'";
    	}
	$query .= "ORDER BY posted ASC LIMIT 1";
	...
	if ($query_result)	 {
		...
		return "<a class='prev' href='http://cgnetswara.org/stanford/single_new.php?id=".$prev_id."'><span class='icon-wrap'><i class='fa fa-angle-left'></i></span><div><h3>$prev_title</h3><img src=$prev_image alt=''/></div></a>";
	}
	else
		return '';
...
function nextpost() {
	global $postings, $currentid, $settings;
	$query = "SELECT id,title,image_file FROM ".$GLOBALS['prefix']."lb_postings WHERE status = 3 AND posted < '".$postings[$currentid]['posted']."' ORDER BY posted DESC LIMIT 1";
	$query_result = $GLOBALS['lbdata']->GetArray($query);
	if ($query_result)	 {
		...
		return "<a class='next' href='http://cgnetswara.org/stanford/single_new.php?id=".$next_id."'><span class='icon-wrap'><i class='fa fa-angle-right'></i></span><div><h3>$next_title</h3><img src=$next_image alt=''/></div></a>";
...
function permalink_to_post_new ($content) {
    global $settings, $currentid;
    return $settings['url']."/stanford/single_new.php?id={$currentid}";
}
